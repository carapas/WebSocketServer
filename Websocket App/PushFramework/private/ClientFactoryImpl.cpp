...
PUSHFRAMEWORK_API vector<LogicalConnection*> ListClient()
{
    return clientFactoryImpl.getClientList();
}
...
vector<LogicalConnection*> ClientFactoryImpl::getClientList()
{
	vector<LogicalConnection*> ret;
	for(clientMapT::iterator it = clientMap.begin();
		it != clientMap.end(); ++it)
	{
